...
app.post('/howdog', async (req, res) => { // (TO DO) url 변경
    ...
    try {
        ...
        const network = await gateway.getNetwork(mynetwork);
        const contract = network.getContract('howdog');
        // (TO DO) changeCarOwner
        await contract.submitTransaction('Receipt_register', recid, gardian, dinfo, price);
...
//prognosis 
app.post('/howdog/progno', async (req, res) => { // (TO DO) url 변경

    try {
        // (TO DO) client로 부터 파리미터 받기
        const cert = req.body.cert;
        const recid = req.body.recid;
        const proinfo = req.body.proinfo;

        console.log('/howdog-post-' + cert + '-' + recid + '-' + proinfo);
        ...
        const contract = network.getContract('howdog');
        // (TO DO) changeCarOwner
        await contract.submitTransaction('Progno_receipt', recid, proinfo);
        console.log('Transaction has been submitted');
        await gateway.disconnect();
...
//Verify
app.post('/howdog/verify', async (req, res) => { // (TO DO) url 변경
